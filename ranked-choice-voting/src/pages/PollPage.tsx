...
export function PollPage() {
  ...
  if (loading || !pollId) {
    return <p className="py-16 text-center text-sm text-slate-400">{t('loadingPoll')}</p>;
  }
  ...
  if (!poll) {
    return (
      <div className="mx-auto max-w-xl rounded-2xl bg-white/80 p-8 text-center shadow-lg ring-1 ring-slate-900/5 dark:bg-slate-900/70 dark:ring-white/10">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('notFoundDesc')}</p>
        <Link
          to="/"
  ...
  async function submit(slot: 'primary' | 'extra') {
    ...
    try {
      ...
      recordHistory(poll.id, poll.name || t('untitledPoll'), 'voted');
      if (slot === 'primary') {
      ...
      if (code === 'permission-denied') {
        setError(poll.status === 'open' ? t('errSubmitRetry') : t('errPollClosed'));
      } else {
  ...
  return (
    <div className="mx-auto max-w-xl space-y-4">
      <ShareBar pollName={poll.name} highlight={justCreated} />
      ...
      <div className="rounded-2xl bg-white/80 p-6 shadow-lg shadow-slate-200/60 ring-1 ring-slate-900/5 backdrop-blur sm:p-8 dark:bg-slate-900/70 dark:shadow-black/30 dark:ring-white/10">
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          {hidden ? t('resultsHiddenNote') : tn('votesSoFar', ballots.length)}
        </p>
        ...
        <AnimatePresence mode="wait">
          {showRanking ? (
            <motion.div key={mode} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">{t('dragHint')}</p>
              <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">{t('shuffleNote')}</p>
              <div className="mt-3">
              ...
              <div className="mt-4">
                <label htmlFor="voter-name" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
                  {t('yourName')} <span className="font-normal text-slate-400 dark:text-slate-500">{t('optional')}</span>
                </label>
                <input
                  id="voter-name"
                  type="text"
              ...
              >
                {submitting ? t('submitting') : mode === 'editing' ? t('updateVote') : t('submitVote')}
              </button>
              {mode !== 'first' && (
                <button
                  type="button"
        ...
        <div className="mt-6 border-t border-slate-100 pt-4 dark:border-slate-800">
          {resultsReadable ? (
            ...
            <p className="text-center text-sm text-slate-400 dark:text-slate-500">{t('resultsWhenClosed')}</p>
          )}
        </div>
        ...
        {isCreator && (
          <div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-dashed border-slate-300 p-3 dark:border-slate-700">
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {isOpen ? t('creatorNoteOpen') : t('creatorNoteClosed')}
            </p>
            <button
              type="button"
              disabled={statusBusy}
              onClick={() => {
                if (isOpen && !window.confirm(t('closeConfirm'))) return;
            ...
            >
              {statusBusy ? '…' : isOpen ? t('closeVoting') : t('reopenVoting')}
            </button>
          </div>
        )}
      </div>
